...
public class GeneratorUI {
	...
	private static VBox freq() {
		...
		freqField.setValueFactory(new SpinnerValueFactory.IntegerSpinnerValueFactory(0,1000000,1000,10));
		freqField.setEditable(true);
		oldValue = freqField.getValue();
		freqField.getEditor().textProperty().addListener((observable,oldValue,newValue) -> { //needed to update value if typed in (not automatically with spinner)
